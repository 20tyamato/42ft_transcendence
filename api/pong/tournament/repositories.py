...
class TournamentRepository:
    ...
    async def get_participants(cls, tournament_id: int) -> List[TournamentParticipant]:
        ...
        return [
            participant
            async for participant in TournamentParticipant.objects.filter(
    ...
    async def add_participant(
        cls, tournament_id: int, username: str
        ...
        try:
            ...
            return (
                await TournamentMatch.objects.filter(
                    tournament_id=tournament_id, game__isnull=True
                )
    ...
    @classmethod
    @transaction.atomic
    async def create_tournament_brackets(
        cls, tournament_id: int, participants: List[str]
        ...
        for i in range(2):
            semi_match = await TournamentMatch.objects.acreate(
                ...
                player1=await User.objects.aget(
                    username=participants[i * 2]
                ),  # 0,2番目の参加者
                player2=await User.objects.aget(
                    username=participants[i * 2 + 1]
                ),  # 1,3番目の参加者
            )
    ...
    async def record_match_result(
        cls, match_id: int, winner_username: str, score_player1: int, score_player2: int
    ) -> Optional[Game]:
        """試合結果の記録"""
        try:
    ...
    async def advance_winner(
        cls, match_id: int, winner_username: str
    ) -> Optional[TournamentMatch]:
        """勝者を次の試合に進める"""
        try:
            match = await TournamentMatch.objects.aget(id=match_id)
    ...
    async def update_tournament_status(
        cls, tournament_id: int, status: str, current_round: int = None
    ) -> Optional[TournamentGameSession]:
        """トーナメントの状態を更新"""
        try:
            tournament = await TournamentGameSession.objects.aget(id=tournament_id)
    ...
    async def get_tournaments(
        cls, status: Optional[str] = None, limit: int = 10
    ) -> List[TournamentGameSession]:
        """トーナメント一覧を取得
        Args:
