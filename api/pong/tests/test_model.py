...
class GameModelTests(TestCase):
    ...
    def test_game_string_representation(self):
        ...
        expected_str = f"Multiplayer: {self.user1.display_name} vs {self.user2.display_name}"
...
class TournamentModelTests(TestCase):
    ...
    def test_tournament_with_participants(self):
        ...
        # 4人の参加者を追加
        TournamentParticipant.objects.create(tournament=tournament, user=self.user1, bracket_position=1)
        TournamentParticipant.objects.create(tournament=tournament, user=self.user2, bracket_position=2)
        TournamentParticipant.objects.create(tournament=tournament, user=self.user3, bracket_position=3)
        TournamentParticipant.objects.create(tournament=tournament, user=self.user4, bracket_position=4)
        
        self.assertEqual(tournament.participants.count(), 4)
        
        # bracket_positionでフィルタリングできることを確認
        semi_final1_players = tournament.participants.filter(bracket_position__in=[1, 2])
